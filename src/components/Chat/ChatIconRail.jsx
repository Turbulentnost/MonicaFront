...
export function ChatIconRail({ user, onLogout, specialMode = false, backMode = false }) {
  const railClass = [
    'chat-rail',
  ...
  return (
    <nav className={railClass} aria-label="Навигация">
      ...
      >
        <span
          className={[
            'chat-rail__logo-mark',
            specialMode ? 'chat-rail__logo-mark--dev' : '',
            backMode ? 'chat-rail__logo-mark--back' : '',
          ]
            .filter(Boolean)
            .join(' ')}
        >
          {backMode ? '∴' : specialMode ? '</>' : 'M'}
        </span>
      </div>
      ...
      <div className="chat-rail__nav">
        ...
        <RailIcon label={backMode ? 'Тишина' : 'Уведомления'}>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" aria-hidden="true">
            <path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9" strokeLinecap="round" strokeLinejoin="round" />
            <path d="M13.7 21a2 2 0 0 1-3.4 0" strokeLinecap="round" />
          </svg>
        </RailIcon>
        <RailIcon label={backMode ? 'Конец' : 'Настройки'}>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" aria-hidden="true">
